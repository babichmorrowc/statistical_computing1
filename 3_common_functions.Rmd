...
```{r 2_loop_speed}
tictoc::tic()
sample <- mvn_generator_2_for_loops(dimension = 2, mu = plot_mu, Sigma = plot_Sigma, n_samples = 10000)
tictoc::toc()
```
...
```{r 1_loop_speed}
tictoc::tic()
sample <- mvn_generator_1_for_loop(dimension = 2, mu = plot_mu, Sigma = plot_Sigma, n_samples = 10000)
tictoc::toc()
```
...
mvn_generator_vector <- function(dimension, mu, Sigma, n_samples) {
  ...
  y_samples <- matrix(rnorm(n = dimension*n_samples,
                      ...
                      nrow = dimension,
                      ncol = n_samples)
  x_samples <- U %*% y_samples + matrix(rep(mu, n_samples), nrow = dimension, ncol = n_samples)
...
```{r vector_speed}
tictoc::tic()
sample <- mvn_generator_vector(dimension = 2, mu = plot_mu, Sigma = plot_Sigma, n_samples = 10000)
tictoc::toc()
```
...
mvn_generator_apply <- function(dimension, mu, Sigma, n_samples) {
  ...
  # Create a matrix where the first column is the means and the second is the standard deviations
...
```{r apply_speed}
tictoc::tic()
sample <- mvn_generator_apply(dimension = 2, mu = plot_mu, Sigma = plot_Sigma, n_samples = 10000)
tictoc::toc()
```
...
Alternatively, the `foreach` and `doParallel` packages can also be used to evaluate R code in parallel using a set number of cores.
